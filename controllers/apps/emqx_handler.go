...
func (r *EmqxReconciler) Do(ctx context.Context, instance appsv1beta3.Emqx) (ctrl.Result, error) {
	...
	if len(instance.GetStatus().Conditions) == 0 || !initPlugin {
		...
		emptyPluginsConfig, _ := generateDefaultPlugins(instance, sts)
	...
	_, sts = generateDefaultPlugins(instance, sts)
...
func generateDefaultPlugins(instance appsv1beta3.Emqx, sts *appsv1.StatefulSet) (*corev1.ConfigMap, *appsv1.StatefulSet) {
	...
	cm := &corev1.ConfigMap{
		...
		Data: map[string]string{
			"emqx_modules.conf":           "modules.loaded_file = /mounted/modules/loaded_modules\n",
