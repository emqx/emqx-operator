package service

const (
	EMQX_NAME = "emqx"

	EMQX_LIC_NAME    = "emqx-lic"
	EMQX_LIC_DIR     = "/opt/emqx/etc/emqx.lic"
	EMQX_LIC_SUBPATH = "emqx.lic"

	EMQX_DATA_DIR = "/opt/emqx/data"
	EMQX_LOG_DIR  = "/opt/emqx/log"

	EMQX_LISTENERS__TCP__DEFAULT_NAME = "mqtt"
	EMQX_LISTENERS__TCP__DEFAULT_PORT = 1883

	EMQX_LISTENERS__SSL__DEFAULT_NAME = "mqtts"
	EMQX_LISTENERS__SSL__DEFAULT_PORT = 8883

	EMQX_LISTENERS__WS__DEFAULT_NAME = "ws"
	EMQX_LISTENERS__WS__DEFAULT_PORT = 8083

	EMQX_LISTENERS__WSS__DEFAULT_NAME = "wss"
	EMQX_LISTENERS__WSS__DEFAULT_PORT = 8084

	EMQX_DASHBOARD__LISTENER__HTTP_NAME = "dashboard"
	EMQX_DASHBOARD__LISTENER__HTTP_PORT = 18083

	EMQX_MANAGEMENT__LISTENER__HTTP_NAME = "api"
	EMQX_MANAGEMENT__LISTENER__HTTP_PORT = 8081
)
