...
var _ = Describe("Rebalance Test", Label("rebalance"), Ordered, func() {
	...
	Context("EMQX Rebalance", func() {
		AfterEach(func() {
			cmd := exec.Command("kubectl", "delete", "-f", "config/samples/apps_v2beta1_emqx.yaml")
			_, _ = utils.Run(cmd)
		...
		// 	By("creating EMQX CR")
		// 	cmd := exec.Command(
		// 		"yq", "e", ".spec.image = \"emqx\"", "config/samples/apps_v2beta1_emqx.yaml", "|",
		// 		"kubectl", "apply", "-f", "-",
		// 	)
		// 	_, err := utils.Run(cmd)
		...
		// 	out, err := utils.Run(cmd)
		// 	Expect(err).NotTo(HaveOccurred(), "Failed to get rebalance status")
		// 	Expect(out).To(Equal(appsv2beta1.RebalancePhaseFailed), "Rebalance should be failed")
