	...
	Context("Check statefulset", func() {
		It("Check statefulset", func() {
			for _, emqx := range emqxList() {
				...
				Expect(sts.Spec.Template.Spec.SecurityContext.FSGroupChangePolicy).Should(Equal(emqx.GetSecurityContext().FSGroupChangePolicy))
				Expect(sts.Spec.Template.Spec.SecurityContext.RunAsNonRoot).Should(Equal(emqx.GetSecurityContext().RunAsNonRoot))
