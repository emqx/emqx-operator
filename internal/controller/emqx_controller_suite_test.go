...
var _ = Describe("CRD Validation", func() {
	...
	Context("Replicants update strategy", func() {
		DescribeTable("validates rollout budgets", func(unavailable, surge *intstr.IntOrString, valid bool, field string) {
			...
			Entry("omitted budgets",
				nil, nil, true, ""),
			Entry("non-negative integers",
				ptr.To(intstr.FromInt(1)), ptr.To(intstr.FromInt(0)), true, ""),
			Entry("negative unavailable",
				ptr.To(intstr.FromInt(-1)), nil, false, ".maxUnavailable"),
			Entry("negative surge",
				nil, ptr.To(intstr.FromInt(-1)), false, ".maxSurge"),
			Entry("leading zeros",
				ptr.To(intstr.FromString("00099%")), ptr.To(intstr.FromString("00150%")), true, ""),
			Entry("100 percent unavailable with zero surge",
				ptr.To(intstr.FromString("00100%")), ptr.To(intstr.FromString("00%")), true, ""),
			Entry("100 percent unavailable with omitted surge",
				ptr.To(intstr.FromString("100%")), nil, true, ""),
			Entry("unavailable above 100 percent",
				ptr.To(intstr.FromString("101%")), nil, false, ".maxUnavailable"),
			Entry("surge above 100 percent",
				nil, ptr.To(intstr.FromString("150%")), true, ""),
			Entry("string count unavailable",
				ptr.To(intstr.FromString("1")), nil, false, ".maxUnavailable"),
			Entry("string count surge",
				nil, ptr.To(intstr.FromString("1")), false, ".maxSurge"),
			Entry("negative percentage",
				ptr.To(intstr.FromString("-1%")), nil, false, ".maxUnavailable"),
			Entry("fractional percentage",
				nil, ptr.To(intstr.FromString("1.5%")), false, ".maxSurge"),
			Entry("both integer zero",
				ptr.To(intstr.FromInt(0)), ptr.To(intstr.FromInt(0)), false, ".maxUnavailable"),
			Entry("both percentage zero",
				ptr.To(intstr.FromString("00%")), ptr.To(intstr.FromString("000%")), false, ".maxUnavailable"),
			Entry("mixed zeros",
				ptr.To(intstr.FromInt(0)), ptr.To(intstr.FromString("00%")), false, ".maxUnavailable"),
			Entry("zero unavailable with omitted surge",
				ptr.To(intstr.FromString("00%")), nil, false, ".maxUnavailable"),
			Entry("omitted unavailable with zero surge",
				nil, ptr.To(intstr.FromString("00%")), true, ""),
			Entry("zero unavailable with surge",
				ptr.To(intstr.FromString("00%")), ptr.To(intstr.FromInt(1)), true, ""),
		)
	})
})
